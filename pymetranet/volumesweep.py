...
class Moment:
    ...
    def get_real_from_dn(mom_info: MomentInfo, dn: int) -> float:
        if mom_info.scaletype == MomentInfo.SCALE_TYPE_LINEAR:
            if mom_info.dataformat in [
                MomentInfo.DATA_FORMAT_FIXED_8_BIT,
                ...
                return float("nan")
        elif mom_info.scaletype == Momen8tInfo.SCALE_TYPE_LOG:
            if mom_info.dataformat in [
                MomentInfo.DATA_FORMAT_FIXED_8_BIT,
                MomentInfo.DATA_FORMAT_FIXED_16_BIT,
            ]:
                # apply same formula for 8bit or 16bit data
                return mom_info.factora + mom_info.factorc * math.pow(
                    10, (1 - dn) / mom_info.factorb
                )
            else:
                # can't do this if format is float
                return float("nan")
...
class PolarSweepInfo:
    ...
    def set(self, sweep: PolarSweep):
        ...
        if mom_info is None:
            raise ValueError(
                "can't get information of moment W to detect if width is normalized"
            )
        self._is_width_norm = (
            mom_info is None if False else self.__detect_width_normalized(mom_info)
        )
        ...
        if mom_info is None:
            raise ValueError(
                "can't get information of moment W to detect if velocity is normalized"
            )
        self._is_vel_norm = (
            mom_info is None if False else self.__detect_velocity_normalized(mom_info)
        )
        ...
        if mom_info is None:
            raise ValueError(
                "can't get information of moment PHIDP to detect if phase is normalized"
            )
        self._is_phidp_norm = (
            mom_info is None if False else self.__detect_phidp_normalized(mom_info)
        )
    ...
    def __get_fields_for_nyquist(self, sweep: PolarSweep) -> int:
        ...
        sweep_data = parse_sweep_data(meta_data)
        ...
        if sweep_data.rsp_cmd.pol == "4 1":
            self.m_PolMode = PolMode.H
        elif sweep_data.rsp_cmd.pol == "4 2":
            self.m_PolMode = PolMode.V
        elif sweep_data.rsp_cmd.pol == "4 3":
            self.m_PolMode = PolMode.HV
        elif sweep_data.rsp_cmd.pol == "4 4":
            self.m_PolMode = PolMode.HHV
        elif sweep_data.rsp_cmd.pol == "2 3":
            self.m_PolMode = PolMode.SIM_HV
