...
class MapSizeRect:
    def __init__(
        self, x_size: int = 0, y_size: int = 0, x_res: float = 0.0, y_res: float = 0.0
    ):
        self._x_size = x_size
        self._y_size = y_size
        self._x_res = x_res
...
class ParamMapSizeRectConverter:
    ...
    def from_string(value: str) -> MapSizeRect:
        ...
        pos_end = value.find(";", pos_start)
        y_size = (
            ParamIntConverter.from_string(value[pos_start:pos_end])
            if pos_end != -1
            else 0
        )
        pos_start = pos_end + 1
        pos_end = value.find(";", pos_start)
        x_res = (
            ParamFloatConverter.from_string(value[pos_start:pos_end])
            if pos_end != -1
            else 0.0
        )
        pos_start = pos_end + 1
        y_res = (
            ParamFloatConverter.from_string(value[pos_start:]) if pos_end != -1 else 0.0
        )
...
class ProductParam(ABC):
    def __init__(
        ...
        name: str,
        descr: str,
        param_type: ProductParamType,
        group: ProductParamGroup,
    ):
        self._name = name
        self._descr = descr
...
class ProductParamHidden(ProductParam):
    def __init__(
        ...
        group: ProductParamGroup,
        value: str,
        default: ProductParamDefault[str],
    ):
...
class ProductParamBool(ProductParam):
    def __init__(
        ...
        group: ProductParamGroup,
        value: bool,
        default: ProductParamDefault[bool],
    ):
...
class ProductParamInt(ProductParam):
    def __init__(
        ...
        group: ProductParamGroup,
        value: int,
        default: ProductParamDefault[int],
        param_range: ProductParamRange[int],
    ):
...
class ProductParamFloat(ProductParam):
    def __init__(
        ...
        group: ProductParamGroup,
        value: float,
        default: ProductParamDefault[float],
        param_range: ProductParamRange[float],
    ):
...
class ProductParamMapSizeRect(ProductParam):
    def __init__(
        ...
        group: ProductParamGroup,
        value: MapSizeRect,
        default: ProductParamDefault[MapSizeRect],
        param_range: ProductParamRange[MapSizeRect],
    ):
...
class ProductInfo:
    def __init__(
        self, product_name: str = None, version: str = None, descr: str = None
    ):
...
class ProductParamInfo:
    def __init__(
        self, value=None, param_type: ProductParamType = ProductParamType.Undefined
    ):
        self._value = value
        self._param_type = param_type
...
class ProdParamsBase(ABC):
            ...
            if info.param_type != ProductParamType.Undefined:
                if info.param_type == ProductParamType.Hidden:
                    ...
                    if name == "time":
                        ...
                        info.value = ParamStringConverter.from_string(value)
                        ret_counter += 1
                elif info.param_type in [
                    ProductParamType.Pid,
                    ProductParamType.String,
                    ProductParamType.Enum,
                    ProductParamType.PathDir,
                    ProductParamType.PathFile,
                    ProductParamType.CatchmentFile,
                    ProductParamType.RadarNetwork,
                ...
                if info.param_type == ProductParamType.Hidden:
                    ...
                    if not param.multi and not param.custom:
                        if param_value not in param.enum_values:
                            ...
                            raise ValueError(
                                f"parameter '{param_name}': invalid value '{param_value}' for enum type. accepted values ({acceptedValues})"
                            )
                    ...
                    if param_value not in ["0", "1"]:
                        raise ValueError(
                            f"parameter '{param_name}': invalid value '{param_value}' for bool type. accepted values (0|1)"
                        )
                    ...
                    if not re.match(r"^-?\d+$", param_value):
                        raise ValueError(
                            f"parameter '{param_name}': invalid value '{param_value}' for int type. not an integer number"
                        )
                    ...
                    if param_range.has_range:
                        if not self.is_in_range(
                            value, param_range.val_min, param_range.val_max
                        ):
                            str_min, str_max = self.get_param_range(param)
                            rangeValues = "{%s}-{%s}" % (str_min, str_max)
                            raise ValueError(
                                f"parameter '{param_name}': invalid value '{param_value}' for range '{rangeValues}'"
                            )
                    ...
                    if not re.match(r"^-?\d+(\.\d*)?$", param_value):
                        raise ValueError(
                            f"parameter '{param_name}': invalid value '{param_value}' for float type. not a floating point number"
                        )
                    ...
                    if param_range.has_range:
                        if (
                            not self.is_in_range(
                                value.x_size,
                                param_range.val_min.x_size,
                                param_range.val_max.x_size,
                            )
                            or not self.is_in_range(
                                value.y_size,
                                param_range.val_min.y_size,
                                param_range.val_max.y_size,
                            )
                            or not self.is_in_range(
                                value.x_res,
                                param_range.val_min.x_res,
                                param_range.val_max.x_res,
                            )
                            or not self.is_in_range(
                                value.y_res,
                                param_range.val_min.y_res,
                                param_range.val_max.y_res,
                            )
                        ):
                            str_min, str_max = self.get_param_range(param)
                            rangeValues = "{%s}-{%s}" % (str_min, str_max)
    ...
    def get_param_info(self, param_name: str) -> ProductParamInfo:
        ret = self._map_param_info.get(
            param_name, ProductParamInfo(None, ProductParamType.Undefined)
        )
        return self._map_param_info.get(
            param_name, ProductParamInfo(None, ProductParamType.Undefined)
        )

    def get_param_string(self, param_info):
        if param_info.param_type in [
            ProductParamType.Pid,
            ProductParamType.String,
            ProductParamType.Enum,
            ProductParamType.PathDir,
